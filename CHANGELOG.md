...
* **Known issue:** the podspecs are excluded from the published package (`.pubignore`), so a pub.dev consumer with Swift Package Manager disabled has no podspec and cannot build for iOS/macOS. Building from the repository with CocoaPods works — verified on CocoaPods 1.17.0, including the podspec's `prepare_command`, which generates the native sources. Use Swift Package Manager (Flutter 3.44+). Details: `doc/wiki/guides/platforms.md`.
